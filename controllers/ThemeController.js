...
const {Theme, Product, Box, Combination, CombinationProduct} = require("../models/index.js");
...
const ThemeController = {
				async create(req, res) {
								try {
												...
												const combinations = await Combination.create({BoxId, ThemeId: theme.id});
												res
																.status(201)
				...
				async getAll(req, res) {
								try {
												const themes = await Theme.findAll({
																...
																attributes: ["id", "theme_name", "description"]
												});
												res.send(themes);
								} catch (error) {
												console.error(error);
				...
				async getByName(req, res) {
								try {
												...
												res
																.status(500)
																.send({message: "An error occurred while getting the theme by name."});
								}
				},
